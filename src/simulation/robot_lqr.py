...
class RobotLqr:
    ...
    def set_velocity_linear_set_point(self, vel: float) -> None:
        """
        Smoothly updates the target velocity of the robot.
        """
        alpha = 0.05  # Smaller = smoother transition
        self.velocity_linear_set_point = (1 - alpha) * self.velocity_linear_set_point + alpha * vel
    ...
    def calculate_lqr_velocity(self) -> float:
        ...
        self.pitch_dot_filtered = self.pitch_dot_filtered * 0.97 + pitch_dot * 0.025
        self.velocity_angular_filtered = self.velocity_angular_filtered * 0.975 + self.get_wheel_velocity() * 0.025
